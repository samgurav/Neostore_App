...
import { AddAddress, getAll,getAddress, DeleteAddress, UpdateAddress } from '../config/MyService';
...
function Address() {
    ...
    const [Errors,SetError]=useState({
      ...
      //set data from token and address from backend
      useEffect(() => {
    ...
    const validate=(errors)=>{
        ...
        Object.values(errors).forEach((val)=> 
            ...
            //on submit sending address data to backend
            const submit=(e)=>{
                e.preventDefault();
                setFlag(false)
                setBflag(false)
        ...
        AddAddress({address:data.address,pincode:data.pincode,state:data.state, city:data.city,country:data.country,uid:data.uid}).then(res=>{
                        if(res.data.err==1) {
                            alert(res.data.message)
                        }
            ...
            UpdateAddress({address:edit.address,pincode:edit.pincode,city:edit.city,state:edit.state,country:edit.country,address_id:address[indexvalue].address_id,uid:data.uid}).then(res=>{  
               console.log(res.data.address)
            alert(res.data.message)
           setFlag(false)
           setBflag(false)
           setTemp(prev=>!prev);
            ...
            const DeleteData = (index) => {
                ...
                DeleteAddress({address:address,uid:data.uid}) .then(res=>{
                    if(res.data.err==1) {
                        alert(res.data.message)
                    }
                    console.log(res.data.address)
        ...
        {flag ? <>
            <div className="container card " style={{padding:'30px'}}>
                    ...
                    <span style={{color:"red"}}>{Errors.country}</span>} 
              {
                    ...
                    <Button onClick={submit} variant='primary'  >Save</Button>
                    </div>:<div>
                    <div className="buttons" style={{marginTop:'50px'}}>
                    <Button onClick={EditAddress } variant='warning'  >Update</Button>
                     </div>
                    </div>
            }
                    <br/>
                    <div className="mb-2"><Button variant="contained-light" onClick={()=>{setFlag(false);setBflag(false)}}   fullwidth>&nbsp;Cancel</Button></div>
                    </Form>
                     </div>
                    </>:<div className='card' style={{padding:'50px'}}>
                    <Button  className="btn btn-light" onClick={()=> {setFlag(true);setEditData({address:'', pincode: '',country:'', state:'', city:''})}} >
                    <HomeIcon />&nbsp;Add Address</Button>
                    <div>
                   <div className="container " >
                    <div>
                                ...
                                <Accordion.Body>
                                    ...
                                    <Button  className="btn btn-warning" onClick={()=> { SetIndex(index);setBflag(true);setFlag(true);editDataFunc({address:ele.address,pincode:ele.pincode,state:ele.state,city:ele.city,country:ele.country})}} ><EditIcon  />&nbsp;Edit</Button>&nbsp;&nbsp;
